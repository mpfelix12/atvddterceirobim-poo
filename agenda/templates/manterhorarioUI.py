...
class ManterHorarioUI:
    ...
    def listar():
        ...
        if len(horarios) ==0: st.write("Nenhum horário cadastrado")
            ...
            dic = []
            for obj in horarios:
                cliente = View.listar_id(obj.get_id_clienete())
                servico = View.servico_listar_id(obj.get_id_servico())
                if cliente != None: clienete = cliente.get_nome()
                if servico != None: servico = servico.get_decricao()
    ...
    def inserir():
        ...
        if st.button("Inserir"):
            ...
            st.success("Horário onserido com sucesso")
